...
MODEL = "gpt-4o-mini"  # Model used for LLM-as-a-judge evaluation
db_name = "vector_db"

hybrid_Retriever = HybridRetriever()

def fetch_context(question: str) -> list[any]:
    """
    Retrieve relevant context documents for a question.
    """
    return hybrid_Retriever.retrieve_context(question)


def answer_query(query: str, history_messages: list = None) -> tuple[str, list]:
    return hybrid_Retriever.answer_query(query, chat_history=history_messages)
...
def evaluate_answer(test: TestQuestion) -> tuple[AnswerEval, str, list]:
    ...
    generated_answer, retrieved_docs = answer_query(test.question)
...
def main():
    ...
    # Explicitly close the client before the interpreter shuts down


if __name__ == "__main__":
    main()
    hybrid_Retriever.client.close()
